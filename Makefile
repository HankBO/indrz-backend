...
setup-indrz_db: ## Setup Indrz DB
	docker cp devops/docker/local/indrz_api/db_init.sql indrz_db:/scripts/db_init.sql
	docker exec -u postgres indrz_db psql indrzdev postgres -f /scripts/db_init.sql
...
load-data-dev: ## load data
	...
	docker exec -it indrz_api python3 manage.py makemigrations
	...
	docker exec -t indrz_api python3 manage.py loaddata --app buildings building_floors.json
	docker exec -t indrz_api python3 manage.py loaddata --app buildings building_spaces.json
	docker exec -t indrz_api python3 manage.py loaddata --app buildings building_wings.json
	docker exec -t indrz_api python3 manage.py loaddata --app buildings initial_ltspacetype_data.json
	docker exec -t indrz_api python3 manage.py loaddata --app poi_manager initial_poi_icons.json
